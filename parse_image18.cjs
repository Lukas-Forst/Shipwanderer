...
const img = new Image();
img.src = fs.readFileSync('public/ship/tile006.png');
const w = img.width;
const h = img.height;
...
function isBgColor(idx) {
  for (const p of bgPalette) {
    const dr = data[idx * 4] - p[0]; const dg = data[idx * 4 + 1] - p[1]; const db = data[idx * 4 + 2] - p[2];
...
for (let i = 0; i < n; i++) if (!visited[i]) unclearedCount++;
console.log("Uncleared pixels count for tile006:", unclearedCount);
